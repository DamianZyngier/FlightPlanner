...
class FlightPlanner {
    ...
    getElements() {
        ...
        return {
            ...
            filterDaysOffCitybreak: q('filter-daysoff-citybreak'), valMaxDaysOffCitybreak: q('val-max-daysoff-citybreak'),
    ...
    bindEvents() {
        ...
        sliders.forEach(key => {
            ...
            el.oninput = (e) => {
                ...
                else if (key === 'filterDaysOffCitybreak' && this.els.valMaxDaysOffCitybreak) this.els.valMaxDaysOffCitybreak.innerText = val;
    ...
    applyConfigToUi() {
        ...
        if (this.els.filterDaysOffCitybreak) this.els.filterDaysOffCitybreak.value = s.max_daysoff_citybreak ?? 0;
    ...
    async saveConfigLocally() {
        ...
        this.data.config.SETTINGS = {
            ...
            max_daysoff_citybreak: parseInt(this.els.filterDaysOffCitybreak?.value || 0),
    ...
    async triggerPrecisionScan(origin, dest, dep, ret, btn) {
        const oldText = btn.innerText; btn.disabled = true; btn.innerText = "⏳ Precision...";
        const isStatic = window.location.hostname.includes('github.io');
        const apiBase = isStatic ? (this.data.config?.SETTINGS?.api_url || '') : '';
        try {
    ...
    renderOriginList() {
        ...
        const html = Object.entries(this.data.config.ORIGINS || {}).filter(([c, d]) => d <= maxVal).sort((a,b) => a[1]-b[1]).map(([c, d]) => { const name = this.getName(c); return `<span class="origin-tag" title="${name}">${name} (${c})</span>`; }).join(' ');
    ...
    renderDashboard() {
        ...
        const priceLongCont = document.getElementById('filter-price-longhaul-container');
        const priceCityCont = document.getElementById('filter-price-citybreak-container');
        const daysOffCityCont = document.getElementById('filter-daysoff-citybreak-container');
        ...
        if (priceLongCont) priceLongCont.style.display = isCityBreak ? 'none' : 'flex';
        if (priceCityCont) priceCityCont.style.display = isCityBreak ? 'flex' : 'none';
        if (daysOffCityCont) daysOffCityCont.style.display = isCityBreak ? 'flex' : 'none';
        ...
        const currentMaxPrice = isCityBreak ? maxPriceCity : maxPriceLong;
        let filtered = this.data.flights.current_best.filter(f => !f.is_mock && f.price <= currentMaxPrice);

        if (this.currentView === 'longhaul') {
            filtered = filtered.filter(f => {
                const lcc = ['FR', 'W6'];
                if (f.all_airlines && f.all_airlines.some(a => lcc.includes(a))) return false;
        ...
        const rescored = filtered.map(f => {
            ...
            return { ...f, ui_score: Math.round((p_s*w.p + w_s*w.we + e_s*w.ef) / totalW) };
        });
        ...
        if (this.els.dealsGrid) {
            ...
            rescored.forEach(f => {
                const card = document.createElement('div'); card.className = 'card'; card.innerHTML = this.createCardHtml(f);
    ...
    renderHistory() {
        ...
        const colors = ['#6366f1', '#10b981', '#ef4444', '#f59e0b', '#8b5cf6', '#06b6d4', '#ec4899', '#f97316', '#84cc16', '#14b8a6', '#3b82f6', '#d946ef', '#fbbf24', '#4ade80', '#fb7185'];
        const datasets = countries.map((c, i) => ({ label: this.getName(c), data: hist.map(h => h.stats?.[c]?.avg || null), borderColor: colors[i % colors.length], backgroundColor: colors[i % colors.length], tension: 0.3, fill: false }));
